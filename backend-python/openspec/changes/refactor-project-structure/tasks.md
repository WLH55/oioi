...
### 4.1 Dramas 模块
- [ ] 4.1.1 创建 `src/dramas/` 目录结构
- [ ] 4.1.2 迁移所有剧目相关代码
- [ ] 4.1.3 **重写所有路由使用 ApiResponse 格式**
- [ ] 4.1.4 **统一路由为 GET/POST 方法**
- [ ] 4.1.5 **添加完整接口文档**
- [ ] 4.1.6 **使用标准分页参数**
- [ ] 4.1.7 实现 service.py 业务逻辑层
- [ ] 4.1.8 实现 dependencies.py（如 valid_drama_id）
- [ ] 4.1.9 创建模块特定异常（DramaNotFound 等）
- [ ] 4.1.10 更新导入和路由
- [ ] 4.1.11 创建测试并验证

### 4.2 Episodes 模块
- [ ] 4.2.1 创建 `src/episodes/` 目录结构
- [ ] 4.2.2 迁移所有集数相关代码
- [ ] 4.2.3 **重写所有路由符合 API 规范**
- [ ] 4.2.4 **实现链式依赖和异常处理**
- [ ] 4.2.5 实现依赖注入（可复用 dramas 的依赖）
- [ ] 4.2.6 实现链式依赖验证（drama -> episode）
- [ ] 4.2.7 更新导入和路由
- [ ] 4.2.8 创建测试并验证

### 4.3 Scenes 模块
- [ ] 4.3.1 创建 `src/scenes/` 目录结构
- [ ] 4.3.2 迁移所有场景相关代码
- [ ] 4.3.3 **重写所有路由符合 API 规范**
- [ ] 4.3.4 实现依赖注入（episode -> scene）
- [ ] 4.3.5 更新导入和路由
- [ ] 4.3.6 创建测试并验证
...
### 5.1 Tasks 模块
- [ ] 5.1.1 创建 `src/tasks/` 目录结构
- [ ] 5.1.2 迁移任务管理代码
- [ ] 5.1.3 **重写所有路由符合 API 规范**
- [ ] 5.1.4 实现依赖注入和验证
- [ ] 5.1.5 更新导入和路由
- [ ] 5.1.6 创建测试并验证

### 5.2 Storyboards 模块
- [ ] 5.2.1 创建 `src/storyboards/` 目录结构
- [ ] 5.2.2 迁移分镜管理代码
- [ ] 5.2.3 **重写所有路由符合 API 规范**
- [ ] 5.2.4 **统一分页和列表响应格式**
- [ ] 5.2.5 实现 service.py 业务逻辑
- [ ] 5.2.6 实现依赖注入验证
- [ ] 5.2.7 更新导入和路由
- [ ] 5.2.8 创建测试并验证

### 5.3 Script Generation 模块
- [ ] 5.3.1 创建 `src/script_generation/` 目录结构
- [ ] 5.3.2 迁移剧本生成代码
- [ ] 5.3.3 **重写所有路由符合 API 规范**
- [ ] 5.3.4 **使用 HttpClientException 处理 AI 调用错误**
- [ ] 5.3.5 实现业务逻辑和依赖注入
- [ ] 5.3.6 更新导入和路由
- [ ] 5.3.7 创建测试并验证
...
### 6.1 Images 模块
- [ ] 6.1.1 创建 `src/images/` 目录结构
- [ ] 6.1.2 迁移图片生成和管理代码
- [ ] 6.1.3 **重写所有路由符合 API 规范**
- [ ] 6.1.4 创建模块配置（AI 服务相关）
- [ ] 6.1.5 更新导入和路由
- [ ] 6.1.6 创建测试并验证

### 6.2 Audio 模块
- [ ] 6.2.1 创建 `src/audio/` 目录结构
- [ ] 6.2.2 迁移音频管理代码
- [ ] 6.2.3 **重写所有路由符合 API 规范**
- [ ] 6.2.4 更新导入和路由
- [ ] 6.2.5 创建测试并验证

### 6.3 Videos 模块
- [ ] 6.3.1 创建 `src/videos/` 目录结构
- [ ] 6.3.2 迁移视频生成和管理代码
- [ ] 6.3.3 **重写所有路由符合 API 规范**
- [ ] 6.3.4 创建模块配置和工具函数
- [ ] 6.3.5 更新导入和路由
- [ ] 6.3.6 创建测试并验证

### 6.4 Video Merges 模块
- [ ] 6.4.1 创建 `src/video_merges/` 目录结构
- [ ] 6.4.2 迁移视频合并代码
- [ ] 6.4.3 **重写所有路由符合 API 规范**
- [ ] 6.4.4 更新导入和路由
- [ ] 6.4.5 创建测试并验证

### 6.5 Upload 模块
- [ ] 6.5.1 创建 `src/upload/` 目录结构
- [ ] 6.5.2 迁移上传处理代码
- [ ] 6.5.3 **重写所有路由符合 API 规范**
- [ ] 6.5.4 更新导入和路由
- [ ] 6.5.5 创建测试并验证

## 7. 共享服务迁移

- [ ] 7.1 创建 `src/ai/` 模块用于共享 AI 服务
- [ ] 7.2 迁移 `app/services/ai_base.py` 到 `src/ai/client.py`
- [ ] 7.3 迁移 `app/services/ai_factory.py` 到 `src/ai/factory.py`
- [ ] 7.4 迁移 AI 提供商实现（openai, doubao）
- [ ] 7.5 **更新 AI 客户端使用 HttpClientException**
- [ ] 7.6 创建 `src/ffmpeg/` 模块用于视频处理服务
- [ ] 7.7 更新所有模块对 AI 服务的导入

## 8. 清理工作

- [ ] 8.1 删除旧的 `app/` 目录
- [ ] 8.2 删除旧的 `main.py`（已迁移到 `src/main.py`）
- [ ] 8.3 更新 `.gitignore` 文件
- [ ] 8.4 更新启动脚本（start.bat, start.sh）
- [ ] 8.5 更新 README.md 文档
- [ ] 8.6 更新部署相关配置文件
- [ ] 8.7 运行完整测试套件验证所有功能
- [ ] 8.8 代码格式化和检查（使用 ruff）

## 9. 验证和文档

- [ ] 9.1 验证所有 API 端点正常工作
- [ ] 9.2 **验证所有接口返回 ApiResponse 格式**
- [ ] 9.3 **验证异常处理正确转换为 ApiResponse**
- [ ] 9.4 **验证 OpenAPI 文档完整且正确**
- [ ] 9.5 验证数据库操作正常
- [ ] 9.6 验证文件上传和静态文件服务
- [ ] 9.7 编写项目结构说明文档
- [ ] 9.8 **编写 API 规范使用指南**
- [ ] 9.9 更新开发指南
- [ ] 9.10 **与前端团队确认 API 迁移完成**
- [ ] 9.11 团队培训和知识转移
